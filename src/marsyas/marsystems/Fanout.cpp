...
Fanout::myUpdate(MarControlPtr sender)
{
  ...
  if (child_count)
  {
    ...
    if (children_info_[0].enabled)
    {
      ...
      children_info_[0].observation_count = marsystems_[0]->getctrl("mrs_natural/onObservations")->to<mrs_natural>();
    ...
    for (mrs_natural i=1; i < child_count; ++i)
    {
      ...
      if (children_info_[i].enabled)
      {
        ...
        children_info_[i].observation_count = marsystems_[i]->getctrl("mrs_natural/onObservations")->to<mrs_natural>();
...
Fanout::myProcess(realvec& in, realvec& out)
{
  ...
  if (child_count)
  {
    ...
    for (mrs_natural i = 0; i < child_count; ++i)
    {
      mrs_natural child_observation_count = children_info_[i].observation_count;
      ...
      if (children_info_[i].enabled)//enabled child have a non-zero localIndex
      {
        ...
        if(!muted(i))
        {
          marsystems_[i]->process(in, buffer_);

          for (o=0; o < child_observation_count; o++)
            for (t=0; t < onSamples_; t++)
              out(out_observation_offset + o,t) = buffer_(o,t);
        }
